...
export const EditTicket = () => {
    ...
    const { id: pk } = useParams(); // Get ticket ID from URL params
    ...
    useEffect(() => {
        if (!authTokens || !authReady || !pk) return;

        const fetchTicketDetails = async () => {
            try {
                setInitialLoading(true);
                const res = await api.get(`/api/event/ticket-detail/${pk}/`);
                ...
                if (ticketData.deadline) {
                    deadlineDate = new Date(ticketData.deadline);
                }

                // Set the ticket pricing data
                setTicketPricing({
                    event: ticketData.event,
                    ticket_type: ticketData.label || '',
                    description: ticketData.description || '',
                    file: null, // File cannot be pre-filled for security reasons
                    deadline: deadlineDate,
                    amount: ticketData.amount ? parseFloat(ticketData.amount).toString() : '',
                    is_verification_required: ticketData.is_verification_required || false
                });

                // Find and set the selected event once events are loaded
                if (events.length > 0) {
                    const event = events.find(e => e.id === ticketData.event);
                    if (event) {
                ...
                navigate(-1); // Go back if ticket not found
            } finally {
        ...
        fetchTicketDetails();
    }, [authReady, authTokens, api, pk, events]);
    ...
    const updateTicketPricing = async () => {
        ...
        try {
            ...
            const res = await api.put(`/api/event/ticket-detail/${pk}/`, formData);
            const data = res.data;
            console.log('ticket pricing updated:', data);
            ...
            // Success handling
            toast.success("Ticket pricing successfully updated");
            setTimeout(() => {
                navigate('/admin/events'); // or navigate back to tickets list
            }, 1000);

        } catch (err) {
            console.error('Failed to update ticket pricing:', err);
            toast.error('Failed to update ticket pricing. Please try again.');
        } finally {
            setLoading(false);
        }
    };
    ...
    if (initialLoading) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-background via-primary-soft to-background flex items-center justify-center">
            ...
            <div className="min-h-screen bg-gradient-to-br from-background via-primary-soft to-background md:p-20 pt-10 ">
                <div className="max-w-6xl mx-auto space-y-8">
                    ...
                    <div className="text-center mb-8">
                        <h1 className="text-3xl font-bold text-gray-900 mb-2">Edit Ticket Pricing</h1>
                        <p className="text-gray-600">Update the ticket pricing details for your event</p>
                    </div>
                    ...
                    <Card className="shadow-lg border-0 bg-gradient-to-r from-card to-primary-soft/10">
                        <CardHeader>
                            ...
                            <CardDescription>
                                Update the ticket pricing for your event
                            </CardDescription>
                        </CardHeader>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-xs text-gray-500">
                                    Leave empty to keep the current image unchanged
                                </p>
                                {errors.file && (
