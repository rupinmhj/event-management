...
const SubmitAllModal = ({ isOpen, onClose, events, onRefresh }) => {
    ...
    const allRequirements = selectedEventId ?
    ...
    const getCompletedCount = () => {
        return allRequirements.filter(req => submissions[req.id] && !req.submitted).length;
    };

    const getAvailableRequirements = () => {
        return allRequirements.filter(req => !req.submitted);
    };
            ...
            <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto ">
                <div className="sticky top-0 bg-white border-b pb-4 ">
                    ...
                    <p className="text-sm text-gray-600">
                        Fill out all pending requirements in one go. Progress: {getCompletedCount()}/{getAvailableRequirements().length} selected
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    {events.length > 1 && (
                        <Card className="bg-white border border-gray-200">
                            <CardHeader className="pb-3">
                                <CardTitle className="text-lg text-gray-900">Select Event (Optional)</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <Select
                                    value={selectedEventId?.toString() || 'all'}
                                    onValueChange={(value) => setSelectedEventId(value === 'all' ? null : parseInt(value))}
                                >
                                    <SelectTrigger className="bg-white border-gray-200">
                                        <SelectValue placeholder="Show requirements for..." />
                                    </SelectTrigger>
                                    <SelectContent>
                    ...
                    <div className="space-y-4">
                        {getAvailableRequirements().map((requirement) => {
                            const eventTitle = selectedEvent?.title || requirement.eventTitle;
                            const eventIcon = selectedEvent?.icon || requirement.eventIcon;
                            ...
                            return (
                                <Card
                                    key={requirement.id}
                                    className={`bg-white border border-gray-200 shadow-sm transition-all duration-300 ${isCompleted ? 'border-green-300 shadow-md' : ''
                    ...
                    {getAvailableRequirements().length === 0 && (
                        <Card className="bg-green-50 border border-green-200">
                            <CardContent className="p-6 text-center">
                                <Check className="w-12 h-12 text-green-600 mx-auto mb-4" />
                                <h3 className="text-lg font-semibold text-green-800 mb-2">All Requirements Completed!</h3>
                                <p className="text-green-600">You have successfully submitted all requirements for the selected events.</p>
                            </CardContent>
                        </Card>
                    )}
